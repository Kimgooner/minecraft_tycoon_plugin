...
public class FishingDataGUI {
    ...
    private final ChestGui fishingChestGui;

    public FishingDataGUI(JavaPlugin plugin, GlobalController globalController, GlobalGUIController globalGUIController) {
        ...
        InputStream xmlStream = plugin.getResource("gui/datachest/datachest-fishing.xml");
        if (xmlStream == null) {
            throw new IllegalStateException("리소스를 찾을 수 없습니다.");
        }
        fishingChestGui = ChestGui.load(this, xmlStream);
        fishingChestGui.setOnGlobalClick(event -> event.setCancelled(true));
    }

    public void open(Player player) {
        fishingChestGui.show(player);
    }
